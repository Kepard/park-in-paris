...
export const tripSchema = z.object({
  ...
  candidate: z.object({ id: z.string().max(150), street: z.string().max(200), arrondissement: z.number().int().min(1).max(20), coordinates: coordinate, bays: z.array(bay).max(1000), capacity: z.number().min(0).max(10000), sharedCapacity: z.number().min(0).max(10000), drive: route, walk: route, driveMinutes: minutes, walkMinutes: minutes, searchLow: minutes, searchHigh: minutes, totalLow: minutes, totalHigh: minutes, total: minutes, arrival: isoDate, learnedFrom: z.number().int().min(0), reason: z.string().max(500), parkingType: z.enum(["paid", "free", "mixed"]) }),
  survey: z.object({ outcome: z.enum(["here", "elsewhere", "gave-up"]), searchMinutes: z.number().min(0).max(180), streetsTried: z.number().int().min(1).max(50), bayType: z.enum(["ordinary", "delivery", "other"]), note: z.string().max(500), submittedAt: isoDate, actualStreet: z.string().max(300), observedArrival: isoDate }).optional(),
